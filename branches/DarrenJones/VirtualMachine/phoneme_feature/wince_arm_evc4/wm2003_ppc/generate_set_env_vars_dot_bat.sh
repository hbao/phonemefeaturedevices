...
echo "set WM2003_SDK_INCLUDE="`cygpath -dm "C:\Program Files\Windows CE Tools\wce420\POCKET PC 2003\INCLUDE\ARMV4"` >> $OutputFile
echo "set WM2003_SDK_LIB="`cygpath -dm "C:\Program Files\Windows CE Tools\wce420\POCKET PC 2003\LIB\ARMV4"` >> $OutputFile

echo "set EVC_ARM_INCLUDE=%WM2003_SDK_INCLUDE%" >> $OutputFile
echo "set EVC_ARM_LIB=%WM2003_SDK_LIB%" >> $OutputFile
echo "set EVC_ARM_PATH=%EVC_ROOT%/EVC/wce420/bin" >> $OutputFile

echo "Title Windows Mobile 2003 Pocket PC SDK - ARM - eVC++ 4.0" >> $OutputFile

        ...
        if (nextIndex != -1) {
            ...
            if ((dir == Canvas.LEFT || dir == Canvas.UP) && viewable[Y] > 0) {
                    ...
                    (viewable[Y] > itemsCopy[traverseIndexCopy].bounds[Y])) 
                {
					...
					if (viewable[Y] < visRect[Y] + visRect[HEIGHT] - viewport[HEIGHT])
					{
						viewable[Y] = visRect[Y] + visRect[HEIGHT] - viewport[HEIGHT];
					}
                    if (viewable[Y] < 0) {
                        viewable[Y] = 0;
                    }
    ...
    boolean scrollForBounds(int dir, int bounds[]) {
        ...
        switch (dir) {
            ...
            case Canvas.UP:
                ...
                viewable[Y] -= (viewport[HEIGHT] - PIXELS_LEFT_ON_PAGE);
				if (viewable[Y] < bounds[Y] + bounds[HEIGHT] - viewport[HEIGHT])
				{
					viewable[Y] = bounds[Y] + bounds[HEIGHT] - viewport[HEIGHT];
				}
				if (viewable[Y] < 0)
				{
					viewable[Y] = 0;

...
class TextBoxLFImpl extends TextFieldLFImpl implements TextFieldLF {
                      ...
                      TextInfo info) 
    {
		...
		cursor.index = editPos + editLength;
		...
		if (underline)
		{
			Text.paintText(info, g, str, font, fgColor, 0xffffff - fgColor, w, h, offset, options, cursor, editPos, editPos + editLength);
		}
		else
		{
			Text.paintText(info, g, str, font, fgColor, 0xffffff - fgColor, w, h, offset, options, cursor, 0, 0);
		}
	...
	case Canvas.UP:
		...
		{
			keyClicked(dir);
			if (myInfo.cursorLine > 0)
			{
				int xCharPosition = cursor.index - myInfo.lineStart[myInfo.cursorLine];
				int newCursorLine = myInfo.cursorLine - 1;
				cursor.index = myInfo.lineStart[newCursorLine] + xCharPosition;
				if (cursor.index > myInfo.lineEnd[newCursorLine] - 1)
				{
					cursor.index = myInfo.lineEnd[newCursorLine];
				}
				cursor.option = Text.PAINT_USE_CURSOR_INDEX;
	...
	case Canvas.DOWN:
		...
		{
			keyClicked(dir);
			if (myInfo.cursorLine < (myInfo.numLines - 1))
			{
				int xCharPosition = cursor.index - myInfo.lineStart[myInfo.cursorLine];
				int newCursorLine = myInfo.cursorLine + 1;

...
public class Text {
    ...
    public static int paintLine(Graphics g, String str, Font font, int fgColor, 
				int w, int h, TextCursor cursor, int offset, int u1, int u2) {
		Font underLineFont = Font.getFont(Font.FACE_SYSTEM, Font.STYLE_PLAIN | Font.STYLE_UNDERLINED, Font.SIZE_MEDIUM);
        if (w <= 0 || 
	...
	if (u2 - u1 > 0)
	{
		...
		if (u1 > 0)
		{
			g.drawChars(text, 0, u1, myOff, h, Graphics.BOTTOM | Graphics.LEFT);
			myOff += font.charsWidth(text, 0, u1);	
		}		
		g.setFont(underLineFont);
		g.drawChars(text, u1, u2 - u1, myOff, h, Graphics.BOTTOM | Graphics.LEFT);
		myOff += font.charsWidth(text, u1, u2 - u1);
		if (u2 < text.length)
		{
			g.setFont(font);
			g.drawChars(text, u2, text.length - u2, myOff, h, Graphics.BOTTOM | Graphics.LEFT);	
		}	
	}
	else
	{
		g.drawChars(text, 0, text.length, offset, h, Graphics.BOTTOM | Graphics.LEFT);
	}
	...
	{
		if (cursor != null && cursor.option == PAINT_USE_CURSOR_INDEX)
		{
			// adjust info.cursorLine to contain cursor.index
			if (info.cursorLine < info.numLines)
			{
				while (info.cursorLine >= 0 && cursor.index < info.lineStart[info.cursorLine])
				{
					info.cursorLine--;
				}
			}
			...
			{
				while (info.cursorLine < info.numLines && cursor.index > info.lineEnd[info.cursorLine])
				{
					info.cursorLine++;
				}
			}
    ...
    public static void paintText(TextInfo info, Graphics g, String str, 
                  ...
                  int w, int h, int offset, int options,
                  TextCursor cursor, int u1, int u2) {
	...
	while (currentLine < (info.topVis + info.visLines)) {
		...
		if(u2 - u1 > 0 && info.lineStart[currentLine] <= u2 && info.lineEnd[currentLine] >= u1)
		{
			int myOff = offset;
			int startU =  info.lineStart[currentLine] > u1 ? info.lineStart[currentLine] : u1;
			int endU   =  info.lineEnd[currentLine] < u2 ? info.lineEnd[currentLine] : u2;
			
			if(info.lineStart[currentLine] < startU)
			{
				g.setFont(font);
				g.drawChars(text, info.lineStart[currentLine], 
					startU - info.lineStart[currentLine],
					myOff, y,
					Graphics.BOTTOM | Graphics.LEFT);
				 myOff += font.charsWidth(text, info.lineStart[currentLine],startU - info.lineStart[currentLine]);	
			}
			
			g.setFont(underLineFont);
			g.drawChars(text, startU, 
					endU - startU,
					myOff, y,
					Graphics.BOTTOM | Graphics.LEFT);
				myOff += font.charsWidth(text, startU,endU - startU);	
			if(endU < info.lineEnd[currentLine] )
			{
				g.setFont(font);
				g.drawChars(text, endU, 
					info.lineEnd[currentLine] - endU,
					myOff, y,
					Graphics.BOTTOM | Graphics.LEFT);
			}
	    }
    ...
    private static boolean getNextLine(char[] text, Font font, int[] inout) {
        ...
        while (curLoc < text.length) {
            ...
            // reached the end of the line
                ...
                if (!foundBreak) {
                    if ((inout[GNL_OFFSET] > 0) && (inout[GNL_LINE_START] > 0)) {

...
{
	int dur = 1000; // In MR3 no duration is sent. Set 1 second as out max vibrate time
	static_cast<MApplication*>(Dll::Tls())->StartVibrate(dur);
    return KNI_TRUE;
}
...
{
	static_cast<MApplication*>(Dll::Tls())->StartVibrate(0);
    return KNI_TRUE;
}

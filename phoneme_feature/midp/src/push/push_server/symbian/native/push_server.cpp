...
{
    int ret = 0;
	HBufC8* connection8 = HBufC8::New(strlen(str));
    connection8->Des().Copy((unsigned char*)str,strlen(str));
    HBufC* connection = HBufC::New(connection8->Des().Length());
    connection->Des().Copy(connection8->Des());

	if(! static_cast<MApplication*>(Dll::Tls())->RegisterConnection(*connection,KNullDesC(),KNullDesC()))
	{
        ret = -2;
	}
    delete connection;

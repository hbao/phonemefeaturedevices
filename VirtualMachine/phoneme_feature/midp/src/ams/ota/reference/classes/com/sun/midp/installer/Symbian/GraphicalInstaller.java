...
public class GraphicalInstaller extends MIDlet implements CommandListener {
    ...
    private Command iCommandRetry = new Command("Retry", Command.OK, 0);
    private Command iCommandCancel = new Command("Exit", Command.CANCEL, 0);
    ...
    public void commandAction(Command c, Displayable s) {
        if (c == nextCmd) {
            ...
            displayCancelledMessage(cancelledMessage);
            cancelBackgroundInstall();
        } else if (c == iCommandRetry) {    
            installSuite(label, url, storageId, forceUpdate, noConfirmation);
        } else if (c == iCommandCancel) {    
            exit(false);
    ...
    private void installSuiteCommon(String action, String name, String url, int storageId,
        ...
        try {
            debugMessage("installSuiteCommon " + action);
    ...
    private Form displayProgressForm(String action, String name,
		...
		debugMessage("displayProgressForm size " + size);
        if (size <= 0) {
...
*/
    ...
    private void updateStatus(int status, InstallState state) {
		debugMessage("updateStatus " + status);
        ...
        if (status == Installer.DOWNLOADING_JAR) {
            updateProgressForm(null, state.getJarSize(),
                 Resource.getString
                   (ResourceConstants.
    ...
    private void updateProgressForm(String url, int size, String gaugeLabel) {
        ...
        Gauge progressGauge;
        StringItem urlItem;
        debugMessage("updateProgressForm " + url + " " + Integer.toString(size) + " " + gaugeLabel);
        ...
        if (url == null) {
            debugMessage("Adding empty string");
            urlItem = new StringItem("", "");
            progressForm.set(progressUrlIndex, urlItem);
        } else if (url.length() != 0) {
            debugMessage("Adding " + Resource.getString(ResourceConstants.AMS_WEBSITE) + ": " + url);
            urlItem =
                new StringItem(Resource.getString
                               (ResourceConstants.AMS_WEBSITE) + ": ", url);
            progressForm.set(progressUrlIndex, urlItem);
    ...
    private void displayDownloadConfirmation(InstallState state) {
        ...
        try {
			...
			infoForm.setTitle("Update Found");

			item = new StringItem(null, "Download " + name);
...
*/
    ...
    private void showExceptionForm(String aTitle, String aMsg)
    {
        Form exceptionForm = new Form(null);
        exceptionForm.setTitle(aTitle);
        StringItem text = new StringItem(null, aMsg);
        exceptionForm.append(text);
        exceptionForm.addCommand(iCommandRetry);
        exceptionForm.addCommand(cancelCmd);
        exceptionForm.setCommandListener(this);
        display.setCurrent(exceptionForm);
    }
    /**
